# 5FS — 2015 prototype format (legacy)

This is the original 2015 design, preserved for reference. The code currently in
this repository (mkfs.gofs, gofsdump, the Vfs_GoFS class) implements this format.
It is superseded by the v2 specification in the rest of `doc/`.
...
## Known shortcomings (why v2 exists)

* 32-bit timestamps (Y2038)
* no checksums on any metadata
* the AG descriptor — the structure everything else depends on — was never defined
* block pointers are absolute device addresses, which makes shrinking require
  rewriting file metadata
* no directory format, journal format, or extent format was ever specified
