...
#define VFS_STATIC_STRING(_x) _x, sizeof(_x)

class Vfs_Interface {
public:
	virtual vfs_ino_t lookup(vfs_ino_t parent, const char *name, size_t name_len) = 0;
	virtual void forget(vfs_ino_t, unsigned long nlookup);

	virtual int format(const char *name, size_t name_len);

	virtual int mknod(vfs_ino_t parent, const char *name, size_t name_len, mode_t mode, dev_t rdev, uid_t owner, gid_t group);
	virtual int mkdir(vfs_ino_t parent, const char *name, size_t name_len);
	virtual int unlink(vfs_ino_t parent, const char *name, size_t name_len);
	virtual int rmdir(vfs_ino_t parent, const char *name, size_t name_len);

	virtual int symlink(vfs_ino_t parent, const char *name, size_t name_len, const char *link, size_t link_len);
	virtual ssize_t readlink(vfs_ino_t ino, char *target, size_t len);
	virtual int rename(vfs_ino_t parent, const char *name, size_t name_len, vfs_ino_t new_parent, const char *new_name, size_t new_name_len);
	virtual int link(vfs_ino_t ino, vfs_ino_t new_parent, const char *new_name, size_t new_name_len);

	virtual int create(vfs_ino_t parent, const char *name, size_t name_len, mode_t mode, uid_t owner, gid_t group, vfs_ino_t *ino, void **context);
	...
	virtual int readdir(vfs_ino_t ino, char *buffer, size_t size, off_t off, void **context);
...
public:
	Vfs_Linux_Block(const char *filename);

	virtual vfs_ino_t lookup(vfs_ino_t parent, const char *name, size_t name_len);
	virtual int read(vfs_ino_t ino, char *buffer, size_t size, off_t off, void **context);
	virtual int write(vfs_ino_t ino, char *buffer, size_t size, off_t off, void **context);
